 ...
 * tuple with the same value at every indexed position, except every N
 * positions, where it is contains the opposite value.
 *
 */
public class MapleLycantropeData implements
		Cursor<BenchmarkDatum<Tuple<Boolean>, Tuple<Boolean>>> {

	private int day;

	public MapleLycantropeData(int day) {
		this.day = day;
	...
	public BenchmarkDatum<Tuple<Boolean>, Tuple<Boolean>>
		...
		int size = 30 + input_size.nextInt( 10 );
		...
		for ( int i = 0; i < size; i++ ) {
			good_output.add( i % day != 0 ? input.get( i ) : !input.get( i ) );
		}
